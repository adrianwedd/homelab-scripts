...
if ! [[ "$MAX_AGE_DAYS" =~ ^[0-9]+$ ]]; then
    print_error "--max-age must be a non-negative integer"
    exit 1
fi
if [ "$MAX_AGE_DAYS" -gt 3650 ]; then
    print_error "--max-age must be <= 3650"
    exit 1
fi
...
audit_auth_keys() {
    ...
    if [ ! -f "$path" ]; then
        ...
        if [ ${#target_issues[@]} -gt 0 ]; then
            local first=true
            target_issues_json=""
            for it in "${target_issues[@]}"; do
                $first || target_issues_json+=", "
                first=false
                target_issues_json+="\"$it\""
            done
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        [ -z "$key_type" ] && {
            print_warning "$target_user: Unrecognized key line"
