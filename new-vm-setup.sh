...
# Execute a command and append stdout/stderr to the setup log.
run_logged() {
    "$@" >>"$LOG_FILE" 2>&1
}
...
if [ ! -f /etc/os-release ]; then
    if [ "$DRY_RUN" = true ]; then
        print_warning "Cannot detect OS (/etc/os-release missing); continuing in dry-run compatibility mode"
        OS_DISTRO="unknown"
        OS_LIKE="unknown"
    else
...
if command -v apt-get >/dev/null 2>&1; then
    ...
    if [ "$DRY_RUN" = true ]; then
        PKG_MANAGER="unknown"
        print_warning "No supported package manager found; package install preview will be generic"
    else
...
if [ "$DRY_RUN" = true ]; then
    ...
    if [ -n "$PACKAGES" ]; then
        ...
        yum | dnf)
            ...
            ;;
        *)
            echo "  <package-manager> install -y ${PACKAGES//,/ }"
            ;;
        esac
        echo ""
    fi
    ...
    if [ "$HOSTNAME" = "$HOSTNAME_BEFORE" ]; then
        ...
        if [ "$NO_SUDO" = true ]; then
            ...
            if command -v hostnamectl >/dev/null 2>&1; then
                run_sudo_logged hostnamectl set-hostname "$HOSTNAME"
            else
...
if [ -n "$USERNAME" ]; then
    ...
    if id -u "$USERNAME" >/dev/null 2>&1; then
        ...
        if [ "$NO_SUDO" = true ]; then
            ...
            run_sudo_logged useradd -m -s "$SHELL_PATH" "$USERNAME"
            USER_CREATED=true
    ...
    if [ "$NO_SUDO" = true ]; then
        ...
        run_sudo_logged usermod -aG "$SUDO_GROUP" "$USERNAME"
...
if [ -n "$PACKAGES" ]; then
    ...
    apt)
        ...
        if [ "$NO_SUDO" = true ]; then
            ...
            run_sudo_logged apt-get update -y
        ...
        for pkg in "${PKG_ARRAY[@]}"; do
            ...
            if dpkg -s "$pkg" >/dev/null 2>&1; then
                ...
                if [ "$NO_SUDO" = true ]; then
                    ...
                    if run_sudo_logged apt-get install -y --no-install-recommends "$pkg"; then
        ...
        for pkg in "${PKG_ARRAY[@]}"; do
            ...
            if rpm -q "$pkg" >/dev/null 2>&1; then
                ...
                if [ "$NO_SUDO" = true ]; then
                    ...
                    if run_sudo_logged "$PKG_MANAGER" install -y "$pkg"; then
                        PACKAGES_INSTALLED+=("$pkg")
                        print_success "Installed: $pkg"
                    else
                        print_warning "Failed to install: $pkg"
    ...
    if ! command -v git >/dev/null 2>&1; then
        ...
        if [ -d "$DOTFILES_DIR" ]; then
            ...
            if [ "$NO_SUDO" = true ]; then
                ...
                if run_sudo_logged -u "$USERNAME" git clone "$DOTFILES_URL" "$DOTFILES_DIR"; then
